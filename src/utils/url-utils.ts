...
export function getRelativeLocaleUrl(lang: string, path: string) : string { 
    ...
    if(prefixDefaultLocale) {
        return baseUrl(joinUrl("/", lang, path));
    }else {
        if(lang === defaultLocale) return baseUrl(path);
        return baseUrl(joinUrl("/", lang, path));
    }
}

export function getPostUrl(lang: string, routePath: string): string {
    return getRelativeLocaleUrl(lang, `/posts/${routePath}`);
}
