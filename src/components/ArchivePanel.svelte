<script>
  ...
  function getPostTags(post) {
    const tags = Array.isArray(post.data.tags)
      ? post.data.tags.filter(tag => typeof tag === 'string' && tag.length > 0)
      : [];
    return tags.length > 0 ? tags : [post.data.category || 'undefined'];
  }

  // 提取所有标签并去重
  $: categories = [...new Set(sortedPosts.flatMap(getPostTags))]
    .sort((a, b) => a.localeCompare(b, currentLang));

  // 响应式过滤逻辑，匹配任意一个已选标签
  $: filteredPosts = selectedCategories.length > 0
    ? sortedPosts.filter(post => getPostTags(post).some(tag => selectedCategories.includes(tag)))
    : sortedPosts;
  ...
  function getCategoriesFromUrl() {
    ...
    return categoryParam && categoryParam !== 'null'
      ? categoryParam.split(',').filter(Boolean)
      : [];
  }

  onMount(() => {
    selectedCategories = getCategoriesFromUrl();
                ...
                <div class="space-y-2">
                    {#each postsByYear[year] as post (post.id)}
                        <div animate:flip={{ duration: 600 }} in:fade={{ duration: 150 }} out:fade={{ duration: 150 }} >
                            <a 
                                href={getPostUrl(currentLang, post.routePath || post.id)} 
                            ...
                            >
                                ...
                                <span class="hidden md:flex items-center flex-wrap justify-end gap-1 font-mono text-sm text-[var(--text-color-70)]">
                                    <Icon icon="fa6-solid:hashtag" class="mr-1" />
                                    {#each getPostTags(post) as tag}
                                        <span>{tag === 'undefined' ? t("pagecard.uncategorized") : tag}</span>
                                    {/each}
                                </span>
                            </a>
                        </div>
